...
public:
    vector<int> stringIndices(vector<string>& wordsContainer, vector<string>& wordsQuery) {
        ...
        int mn_len = 1e6, idx = -1;
        for (int t = 0; t < wordsContainer.size(); t++) {
            auto &s = wordsContainer[t];
            hash61 H(s);
            if (s.size() < mn_len) idx = t, mn_len = s.size();
            for (int i = 0; i < s.size(); i++) {
                auto cur = H.get_hash(i, s.size() - 1);
                if (mp.count(cur)) {
                    if (mp[cur].second > s.size()) mp[cur] = {t, s.size()};
                } else {
                    mp[cur] = {t, s.size()};
                }
            }
